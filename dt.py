    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                classL = util.mode(leftY)
                classR = util.mode(rightY)
            ...
            if bestFeature < 0:
                ...
                self.left.trainDT(X[X[:,self.feature]<0.5,:],Y[X[:,self.feature]<0.5],self.left.opts['maxDepth'],[self.feature])
                self.right.trainDT(X[X[:,self.feature]>=0.5,:],Y[X[:,self.feature]>=0.5],self.right.opts['maxDepth'],[self.feature])
