...
class LockTable:
	...
	@staticmethod
	def dfs(G, t, paths, t_list):
		#mark visited 
		paths[t] = "gray"
		
		for waiting in G[t]:
			if paths[waiting] == "gray":
				#found cycle
				t_list.append[waiting]
			if paths[waiting] == "white":
				dfs(G, waiting, paths, t_list)
	...
	def detectDeadlocksAndChooseTransactionsToAbort():
		...
		# wait-for graph
		# verticies = all transactions in the system
		# edges = pair: (x waits for y)
		graph = dict()

		# cond = LockTable.condition_objects[obj_id]
		# 	with cond:

		#graph = dict((k, []) for k in lockhashtable)
		
		#construct wait for graph
		for obj_id in lockhashtable:
			entry = LockTable.lockhashtable[oj_id] 
		 	for (waiting_t_id, locktype) in entry.waiting_transactions_and_locks:
		 		for (current_t_id, lock) in entry.current_transactions_and_locks:
		 			if lock == 1:
		 				#check if list is initialized first 
		 				if graph[waiting_t_id]: 
							graph[waiting_t_id].append(current_t_id)
						else:
							graph[waiting_t_id] = [current_t_id]


		#check for cycles with DFS
		#use color graph to keep track of visted 
		deadlock_transactions = []
		paths = { k : "white" for k in graph}
		for t in graph:
			if paths[t] == "white":
				dfs(graph, t, paths, deadlock_transactions)
...
class TransactionState:
	...
	def getXLockTuple(self, relation, primary_id):
		if [relation.fileName, LockTable.IX] not in self.locks and self.getLock(relation.fileName, LockTable.X) not in self.locks:
