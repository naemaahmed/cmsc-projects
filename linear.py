...
class LogisticLoss(LossFunction):
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        gradient of the loss associated with these predictions.
        """ 
        return - sum(Y * X.T, axis=1)
        #return - sum(dot(-Y,Yhat)*X.T*exp(dot(-Y, Yhat))/(1+exp(dot(-Y,Yhat))), axis=1)
        #return - sum((Y - Yhat) * X.T, axis=1)
...
class LinearClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        def func(w):
            ...
            obj  = lossFn.loss(Y, Yhat) + (lambd/2) * linalg.norm(w)  
            return obj
