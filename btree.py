...
class BTreeBlock(Block):
	...
	def redistributeWithBlock(self, otherBlock):
		...
		if self.isLeaf:
			for i in range(0, len(otherBlock.keysAndPointers), 2):
				if otherBlock.keysAndPointers[i] is not None:
					# remove otherBlock's last pointer and last value
					ptr = otherBlock.keysAndPointers.pop(i)
					key = otherBlock.keysAndPointers.pop(i)
					# insert it as the first pointer & value in self, shift everything right 
					addPointer(self, ptr, key)
			
			#replace value in parent in between pointers to self and otherBlock with otherBlock's last value
			# ^done in mergeOrRedistributeWithSibling() so just return the value of otherBlock's last key
			for i in range(1, len(otherBlock.keysAndPointers), 2):
				if otherBlock.keysAndPointers[i] is not None:
					return otherBlock.keysAndPointers[i]
		else:
			for i in range(0, len(otherBlock.keysAndPointers), 2):
				if otherBlock.keysAndPointers[i] is not None:
					#remove otherBlock's last pointer and second to last value
					ptr = otherBlock.keysAndPointers.pop(i)
					key = otherBlock.keysAndPointers.pop(i-1)

			for i in range(1, len(otherBlock.keysAndPointers), 2):
				if otherBlock.keysAndPointers[i] is not None:
					# insert (N′.Pm, K′) as the first pointer and value in N,
					addPointer(self, ptr, otherBlock.keysAndPointers[i])
					# return the value of otherBlock's second to last key
					return otherBlock.keysAndPointers[i-1]
